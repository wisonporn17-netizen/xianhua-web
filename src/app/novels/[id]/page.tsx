  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-5xl mx-auto px-4 pt-6">
        <Link href="/" className="inline-flex items-center gap-1.5 text-gray-400 hover:text-xh-purple2 text-sm transition-colors">
      ...
      <section className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex flex-col sm:flex-row gap-6">
          ...
          <div className="flex-1 min-w-0 text-center sm:text-left">
            <h1 className="text-xl sm:text-2xl font-bold text-white leading-snug mb-1">{novel.title}</h1>
            {novel.subtitle && <p className="text-xh-gold/70 text-sm mb-3">{novel.subtitle}</p>}
            {novel.description && <p className="text-gray-400 text-sm leading-relaxed mb-4">{novel.description}</p>}
            <div className="flex items-center gap-1.5 justify-center sm:justify-start">
              <span className="text-xh-gold text-xs">✦</span>
              <span className="text-gray-400 text-sm">
                <span className="text-white font-semibold">{episodes.length}</span> ตอน
              </span>
            </div>
          </div>
        </div>
          ...
          {episodes.map((episode: any) => (
            <Link key={episode.id} href={`/novels/${novel.id}/episodes/${episode.id}`}
              ...
              <div className="flex-1 min-w-0">
                <p className="text-white text-sm font-medium truncate group-hover:text-xh-purple2 transition-colors">{episode.title}</p>
              </div>
              <svg viewBox="0 0 24 24" className="w-4 h-4 text-gray-600 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
